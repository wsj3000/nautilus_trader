...
//!
//! It has to produce a real execution. The order is one share at market, so it fills promptly
//! during a session and rests unfilled outside one. Run it while SIP or the overnight session is
//! open.
//!
//! Outside a session it reports that it could not get a fill rather than pretending to have
...
use nautilus_alpaca::{
    ...
    http::{
        ...
        query::{ActivitiesParams, SubmitOrderRequest},
    },
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = SubmitOrderRequest {
        ...
        // Market rather than a limit priced through the touch: a limit that crosses still rests if
        // the book moves away, and this check wants a fill or a clear failure, not a maybe.
        order_type: AlpacaOrderType::Market,
        time_in_force: AlpacaTimeInForce::Day,
        client_order_id: Some(client_order_id.clone()),
        limit_price: None,
        stop_price: None,
        // The overnight and pre/post sessions need this, and it is harmless during regular hours.
        extended_hours: false,
    };

    println!("submitting {SYMBOL} buy {QTY} at market");
